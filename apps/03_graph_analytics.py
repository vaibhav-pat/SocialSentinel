...
from pyspark.sql.functions import col, current_timestamp, expr, from_json, lit
...
spark = SparkSession.builder.appName("Graph_Analytics_Risk").getOrCreate()
...
def foreach_batch(df, batch_id):
    ...
    df = df.where(
        col("event_time")
        >= expr(f"timestampadd(HOUR, -{WINDOW_HOURS}, current_timestamp())")
    )
    ...
    user_pr = pr.vertices.filter(col("type") == "USER").selectExpr(
        "id as author_id", "pagerank"
    )
    user_cc = cc.vertices.filter(col("type") == "USER").selectExpr(
        "id as id_cc", "component"
    )
    user_deg = deg.selectExpr("id as id_deg", "degree")

    users_analytics = (
        user_pr.join(user_cc, user_pr.author_id == col("id_cc"), "left")
        .drop("id_cc")
        .join(user_deg, user_pr.author_id == col("id_deg"), "left")
        .drop("id_deg")
    )
    ...
    severe = (
        df.where(col("classification").isin("depression", "anxiety", "stress"))
        .groupBy("author_id")
        .count()
        .withColumnRenamed("count", "severe_cnt")
    )

    # Join and compute risk
    joined = users_analytics.join(severe, "author_id", "left").fillna(
        {"severe_cnt": 0, "pagerank": 0.0, "component": 0, "degree": 0}
    )

    from pyspark.sql.functions import col as c

    risk = (
        joined.withColumn("content_score", (c("severe_cnt") > 0).cast("double"))
        .withColumn(
            "centrality_score", (c("pagerank") / (c("pagerank") + 1e-6))
        )  # scaled 0..~1
        .withColumn(
            "sentiment_spike", lit(0.0)
        )  # placeholder; compute from time-series if indexed
        .withColumn(
            "risk",
            W_CONTENT * c("content_score")
            + W_CENTRALITY * c("centrality_score")
            + W_SPIKE * c("sentiment_spike"),
        )
    ...
    (
        risk.write.format("org.elasticsearch.spark.sql")
